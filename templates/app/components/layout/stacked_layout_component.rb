...
module Layout
  # Layout with navbar at top, collapsible sidebar on mobile
  #
  # @example Full layout
  #   <%= render Layout::StackedLayoutComponent.new do |layout| %>
  ...
  class StackedLayoutComponent < ApplicationComponent
    ...
    def container_classes
      class_names(
        "relative isolate flex min-h-svh w-full flex-col bg-white lg:bg-olive-100",
        "dark:bg-olive-900 dark:lg:bg-olive-950",
        options[:class]
      )
    end
    ...
    def header
      tag.header(class: "flex items-center px-4") do
        safe_join([
          tag.div(class: "py-2.5 lg:hidden") do
            open_menu_button
          end,
    ...
    def main_content
      tag.main(class: main_classes) do
        tag.div(class: content_wrapper_classes) do
          tag.div(content, class: "mx-auto max-w-6xl")
        end
      end
    end
    ...
    def content_wrapper_classes
      class_names(
        "grow p-6 lg:rounded-lg lg:bg-white lg:p-10 lg:shadow-sm lg:ring-1 lg:ring-olive-950/5",
